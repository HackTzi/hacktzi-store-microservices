...
import { ProductDto, ProductGetById, ProductGetBytitleUri, ProductGetResponseDto, ProductQueryDto, sortOptions, UpdateProductDto } from "../dtos/product.dto";
...
export class ProductController {
    ...
    */
    ...
    @Post()
    saveProduct(@Body() body: ProductDto): Promise<ProductDto>{ //TODO: use auth token
        return this.productService.create(body)
    }

    @ApiOperation({description: 'Get all products'})
    @ApiResponse({status: 200, description: 'Products', type: ProductGetResponseDto})
    @ApiQuery({type: ProductQueryDto})
    @Get()
    getAllProducts(@Query() queries: ProductQueryDto): Promise<ProductGetResponseDto>{
        return this.productService.findAll(queries)
    }

    @ApiOperation({description: 'Search - Not Implemented'})
    @ApiResponse({status: 200, description: 'Search results',type: ProductGetResponseDto})
    @ApiQuery({type: ProductQueryDto})
    @Get('search')
    searchProduct(@Query() queries: ProductQueryDto, @Query('q') q: string): Promise<any>{ //TODO: implement
    ...
    @ApiResponse({status: 200, description: 'Product', type: Product})
    @Get(':title_uri')
    getProductByTitleUri(@Param() params: ProductGetBytitleUri): Promise<Product> {
    ...
    @Get('category/:id')
    getProductsByCategoryId(@Param() params: getById, @Query() queries: ProductQueryDto): Promise<ProductGetResponseDto>{
    ...
    @Delete(':id')
    deleteProductById(@Param() params: getByIdParamDto): Promise<any> { //TODO: use auth token
    ...
    @Put(':id')
    updateProductById(@Param() params: getById, @Body() body: UpdateProductDto): Promise<any>{ //TODO: use auth token
    ...
    @ApiQuery({type: ProductQueryDto})
    @Get('merchant/:id')
    getProductsByMerchantId(@Param() params: getByIdParamDto, @Query() queries: ProductQueryDto): Promise<ProductGetResponseDto>{
